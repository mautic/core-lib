...
class PublicController extends CommonFormController
{
    ...
    {
        $data = $this->buildMessengerResponseData($context, $submissionResult);
     ...
     * @return array<string, mixed>
     */
    private function buildMessengerResponseData(array $context, array $submissionResult): array
    {
        $data  = ['success' => 1];
        $error = $submissionResult['error'];

        if (!empty($error)) {
            $data = $this->addMessengerError($data, $error);
        } else {
     ...
     * @return array<string, mixed>
     */
    private function addMessengerError(array $data, mixed $error): array
    {
        if (is_array($error)) {
    ...
    private function addMessengerSuccessData(array $data, array $submissionResult): array
    {
        ...
        $data = $this->addPostActionData(
            $data,
            is_string($submissionResult['postAction']) ? $submissionResult['postAction'] : null,
            $submissionResult['postActionProperty']
        );
     ...
     * @return array<string, mixed>
     */
    private function addPostActionData(array $data, ?string $postAction, mixed $postActionProperty): array
    {
        switch ($postAction) {
            case 'redirect':
    ...
    private function buildStandardResponse(Request $request, array $context, array $submissionResult): Response
    {
        ...
        if (!empty($error)) {
            if ($context['return']) {
                ...
                $response = $this->redirect((string) $context['return'].(string) $context['query'].'mauticError='.rawurlencode((string) $error).$hash);
            } else {
                $msg     = $error;
                $msgType = 'error';
            }
