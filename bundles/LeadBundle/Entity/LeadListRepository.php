...
SQL;
     ...
     * @return mixed[]
     */
    public function getEmailStat(): array
    {
        $rsm = new ResultSetMapping();
        $rsm->addScalarResult('item_id', 'item_id');
        ...
        $query = $this->getEntityManager()->createNativeQuery('SELECT 
            list_id as item_id
        FROM '.MAUTIC_TABLE_PREFIX.'email_stats
            GROUP BY list_id', $rsm);
     ...
     * @return mixed[]
     */
    public function getNotificationStat(): array
    {
        $rsm = new ResultSetMapping();
        $rsm->addScalarResult('item_id', 'item_id');
        ...
        $query = $this->getEntityManager()->createNativeQuery('SELECT 
            list_id as item_id
        FROM '.MAUTIC_TABLE_PREFIX.'push_notification_stats
            GROUP BY list_id', $rsm);
     ...
     * @return mixed[]
     */
    public function getSMSStat(): array
    {
        $rsm = new ResultSetMapping();
        $rsm->addScalarResult('item_id', 'item_id');
        ...
        $query = $this->getEntityManager()->createNativeQuery('SELECT 
            list_id as item_id
        FROM '.MAUTIC_TABLE_PREFIX.'sms_message_stats
            GROUP BY list_id', $rsm);
