...
class EmailRepositoryTest extends TestCase
{
    ...
    {
        yield [null, false, [], "SELECT count(*) as count FROM test_leads l WHERE (EXISTS (SELECT ll.lead_id FROM test_lead_lists_leads ll WHERE (ll.lead_id = l.id) AND (ll.leadlist_id IN (22, 33)) AND (ll.manually_removed = :false))) AND (NOT EXISTS (SELECT dnc.lead_id FROM test_lead_donotcontact dnc WHERE (dnc.lead_id = l.id) AND (dnc.channel = 'email'))) AND (NOT EXISTS (SELECT mq.lead_id FROM test_message_queue mq WHERE (mq.lead_id = l.id) AND (mq.status <> 'sent') AND (mq.channel = 'email'))) AND (NOT EXISTS (SELECT null FROM test_email_stats stat WHERE (stat.email_id = 5) AND (stat.lead_id = l.id))) AND (l.id NOT IN (SELECT lc.lead_id FROM test_lead_categories lc INNER JOIN test_emails e ON e.category_id = lc.category_id WHERE (e.id = 5) AND (lc.manually_removed = 1))) AND ((l.email IS NOT NULL) AND (l.email <> ''))"];
        yield [[6], false, [16], "SELECT count(*) as count FROM test_leads l WHERE (EXISTS (SELECT ll.lead_id FROM test_lead_lists_leads ll WHERE (ll.lead_id = l.id) AND (ll.leadlist_id IN (22, 33)) AND (ll.manually_removed = :false))) AND (NOT EXISTS (SELECT dnc.lead_id FROM test_lead_donotcontact dnc WHERE (dnc.lead_id = l.id) AND (dnc.channel = 'email'))) AND (NOT EXISTS (SELECT mq.lead_id FROM test_message_queue mq WHERE (mq.lead_id = l.id) AND (mq.status <> 'sent') AND (mq.channel = 'email'))) AND (NOT EXISTS (SELECT null FROM test_email_stats stat WHERE (stat.email_id = 6) AND (stat.lead_id = l.id))) AND (NOT EXISTS (SELECT null FROM test_email_stats stat WHERE (stat.email_id = 5) AND (stat.lead_id = l.id))) AND (l.id NOT IN (SELECT lc.lead_id FROM test_lead_categories lc INNER JOIN test_emails e ON e.category_id = lc.category_id WHERE (e.id = 5) AND (lc.manually_removed = 1))) AND ((l.email IS NOT NULL) AND (l.email <> ''))"];
